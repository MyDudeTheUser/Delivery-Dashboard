...
- **`Content-Security-Policy` (CSP):** A strict policy that only allows resources (scripts, styles, images) to load from the same origin (`'self'`). It explicitly whitelists API connections to `https://api.yourdomain.com`. _(Note: Update this URL in `vercel.json` when your real API is deployed)._
...
_Expected Result:_ The command should return Vercel's IP address (`76.76.21.21`) or Vercel's CNAME (`cname.vercel-dns.com`).
